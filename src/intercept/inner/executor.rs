...
pub trait Executor {
    fn run(program: &std::path::Path, args: &[String], envs: &Vars) -> Result<ExitCode>;
}

#[cfg(unix)]
pub fn executor(reporter: Sender<Event>) -> Result<impl Executor> {
    unix::Executor::new(reporter)
}
...
mod unix {
    ...
    impl Executor {
        pub fn new(reporter: Sender<Event>) -> Result<Self> {
            let cwd = env::current_dir()
                .chain_err(|| "Unable to get current working directory")?;

            let executor = Executor { reporter, cwd };
            Ok(executor)
        }
        ...
        fn spawn<F>(&self, program: &std::path::Path, args: &[String], envs: &Vars) -> Result<nix::unistd::Pid>
        {
...
//    }
    ...
    fn execute(program: &std::path::Path, args: &[String], envs: &Vars) -> Result<()> {
        let c_args: Vec<_> = args.iter()
            ...
            .collect();
        let c_envs: Vec<_> = envs.iter()
            .map(|(key, value)| {
                let mut env = ffi::OsString::from(key);
                env.push("=");
                env.push(value);
                ffi::CString::new(env.to_str().unwrap()).unwrap()
            })
            .collect();
        let c_program: ffi::CString = ffi::CString::new(program.as_os_str().to_str().unwrap()).unwrap();
...
mod generic {
    ...
    impl Executor {
        pub fn new(reporter: Sender<Event>) -> Result<Self> {
            let executor = Executor { reporter };
            Ok(executor)
        }
    }
